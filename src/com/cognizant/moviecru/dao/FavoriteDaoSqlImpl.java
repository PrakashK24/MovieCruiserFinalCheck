...
import com.cognizant.moviecru.dao.ConnectionHandler;
...
public class FavoriteDaoSqlImpl implements FavoriteDao {
	...
	public void addFavorite(long userId, long movieId) {
		...
		try {
			if (connection != null) {
				preparedStatement = connection.prepareStatement("insert into favorites values (default, ?, ?)");
	...
	public List<Movies> getAllFavorites(long userId) throws FavoriteEmptyException {
		...
		try {
			...
			if (connection != null) {
				...
				sqlBuffer.append(
						"select mi.mo_id,mi.mo_title,mi.mo_box_office,mi.mo_active,mi.mo_date_of_launch,mi.mo_genre,mi.mo_has_teaser from movies mi ")
	...
	private long getNumOfFavorites(long userId, Connection conn) {
		...
		try {
			if (conn != null) {
				...
				StringBuffer sqlBuffer = new StringBuffer();
				sqlBuffer.append(
						"select count(mo_box_office) from movies inner join favorites on movies.mo_id=favorites.fe_mo_id ")
	...
	public void removeFavorite(long userId, long movieId) {
		...
		try {
			...
			if (conn != null) {
				prepareStatement = conn.prepareStatement("delete from favorites where fe_us_id=? and fe_mo_id=?");
